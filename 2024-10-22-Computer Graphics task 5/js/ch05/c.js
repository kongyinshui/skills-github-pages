...
var scale = [1.0, 1.0, 1.0]; // 添加缩放因子
var scaleLoc;

var rotationSpeed = 0.08; // 旋转速度
var theta = [0, 0, 0]; // 旋转角度
var thetaLoc;
...
window.onload = function initCube() {
    ...
    gl.uniform3fv(scaleLoc, scale); // 将初始缩放因子传递给着色器

    thetaLoc = gl.getUniformLocation(program, "theta"); // 获取旋转角度的 uniform 位置
    gl.uniform3fv(thetaLoc, theta); // 设置初始旋转角度

    document.getElementById("xbutton").onclick = function () {
        scale[0] -= 0.2; // 沿 X 轴缩放，增加更大的步长
        gl.uniform3fv(scaleLoc, scale); // 更新缩放因子
    };
    ...
    document.getElementById("zbutton").onclick = function () {
        scale[2] -= 0.2; // 沿 Z 轴缩放，增加更大的步长
        gl.uniform3fv(scaleLoc, scale); // 更新缩放因子
    };

    document.getElementById("pause").onclick = function () {
        scale = [1.0, 1.0, 1.0]; // 重新设置缩放因子
        gl.uniform3fv(scaleLoc, scale); // 更新缩放因子
    };
...
function render() {
    ...
    gl.uniform3fv(thetaLoc, theta); // 更新旋转角度
